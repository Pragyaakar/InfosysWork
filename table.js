        ...
        function beforeLoad(scriptContext) {
            try {
                ...
                if (scriptContext.type == scriptContext.UserEventType.VIEW) {
                    ...
                    var subsidiary = billRec.getValue({
                        fieldId: 'subsidiary'
                    });

                    if (subsidiary == globalLLCSubsidiary || subsidiary == EllationLLCdbaCrunchyroll || subsidiary == CrunchyrollAustralia) {
                        ...
                        if (isEmpty(billCreatedFrom)) {
                            ...
                            /*var firstApproverField = billForm.getField({id : 'custbody_first_approver'});
					firstApproverField.isMandatory = true;*/
                            var firstApprover = billRec.getValue({
                                fieldId: 'custbody_first_approver'
                            });
                            var secondApprover = billRec.getValue({
                                fieldId: 'custbody8'
                            });
                            var thirdApprover = billRec.getValue({
                                fieldId: 'custbody9'
                            });
                            var fourthApprover = billRec.getValue({
                                fieldId: 'custbody10'
                            });
                            var financeApprover = billRec.getValue({
                                fieldId: 'custbody_crw_finance_approver'
                            });
                            //var fifthApprover  = billRec.getValue({fieldId: 'custbody_crw_fifth_approver'});
                            var billCreator = billRec.getValue({
                                fieldId: 'custbody_createdby'
                            });
                            var stage = billRec.getValue({
                                fieldId: 'custbody_stage'
                            });
                            var rejector = billRec.getValue({
                                fieldId: 'custbody_rejected_by'
                            });
                            var itemLines = billRec.getLineCount({
                                sublistId: 'item'
                            });
                            var expenselines = billRec.getLineCount({
                                sublistId: 'expense'
                            });
                            var approvalStatus = billRec.getValue({
                            ...
                            var suiteletUrl = url.resolveScript({
                                scriptId: suiteletCalled[0],
                                deploymentId: suiteletCalled[1]
                            });
                            ...
                            if ((currentUser.id != billCreator) && (approvalStatus == 1 && ((stage == 0 && (currentUser.id == firstApprover || currentUser.role == '3')) || (stage == 1 && currentUser.id == secondApprover) || (stage == 2 && currentUser.id == thirdApprover) || (stage == 3 && currentUser.id == fourthApprover) || (stage == 4 && financeAppr.indexOf((currentUser.id).toString()) != -1) || ((stage == 1 || stage == 2 || stage == 3 || stage == 4) && currentUser.role == '3'))) || (stage == 4 && (currentUser.id == 764 && (currentUser.role == '1045' || currentUser.role == '1026')) && approvalStatus == 1) || ((currentUser.id == 688 && currentUser.role == '1049') && approvalStatus == 1)) {
                                billForm.addButton({
                                    id: 'custpage_approve',
                                    label: 'Approve',
                                    functionName: "window.open('" + approveUrl + "','_self')"
                                });
                                billForm.addButton({
                                    id: 'custpage_reject',
                                    label: 'Reject',
                                    functionName: "window.open('" + rejectUrl + "','_self')"
                                });
                            } else {
                    ...
                    if (runtime.executionContext != runtime.ContextType.SUITELET && runtime.executionContext != runtime.ContextType.SCHEDULED && scriptContext.type != scriptContext.UserEventType.DELETE) {
                        ...
                        if (subsidiary == globalLLCSubsidiary || subsidiary == EllationLLCdbaCrunchyroll || subsidiary == CrunchyrollAustralia) {
                            ...
                            if (isEmpty(billCreatedFrom)) {
                                ...
                                var emailauthor = '';
                                var isEdited = false;
                                var billRecObj = record.load({
                                    type: recType,
                                    id: recId
                                });
                                var currentUser = runtime.getCurrentUser();
                                var documentNumber;
                                var referenceNumber = billRecObj.getValue({
                                    fieldId: 'tranid'
                                });
                                if (!isEmpty(referenceNumber)) {
                                    documentNumber = referenceNumber;
                                } else {
                                    documentNumber = billRecObj.getValue({
                                        fieldId: 'transactionnumber'
                                    });
                                }
                                var subsidiary = billRecObj.getValue({
                                    fieldId: 'subsidiary'
                                });
                                var billStatus = billRecObj.getValue({
                                    fieldId: 'approvalstatus'
                                });
                                var totalAmount = billRecObj.getValue({
                                    fieldId: 'total'
                                });
                                var costCenter = billRecObj.getValue({
                                    fieldId: 'department'
                                });
                                var firstApprover = billRecObj.getValue({
                                    fieldId: 'custbody_first_approver'
                                });
                                var firstApproverText = billRecObj.getText({
                                    fieldId: 'custbody_first_approver'
                                });

                                var newFirstApprover = billRecObj.getValue({
                                    fieldId: 'custbody_first_approver'
                                });
                                var newSecondApprover = billRecObj.getValue({
                                    fieldId: 'custbody8'
                                });
                                var newThirdApprover = billRecObj.getValue({
                                    fieldId: 'custbody9'
                                });
                                var newFourthApprover = billRecObj.getValue({
                                    fieldId: 'custbody10'
                                });
                                var newFinanceApprover = billRecObj.getValue({
                                    fieldId: 'custbody_crw_finance_approver'
                                });
                                var itemLines = billRecObj.getLineCount({
                                    sublistId: 'item'
                                });
                                var expenselines = billRecObj.getLineCount({
                                    sublistId: 'expense'
                                });
                                ...
                                if (scriptContext.type == scriptContext.UserEventType.EDIT) {
                                    ...
                                    var oldTotal = oldBillRec.getValue({
                                        fieldId: 'total'
                                    });
                                    var oldCostCenter = oldBillRec.getValue({
                                        fieldId: 'department'
                                    });
                                    var oldFirstApprover = oldBillRec.getValue({
                                        fieldId: 'custbody_first_approver'
                                    });
                                    var oldSecondApprover = oldBillRec.getValue({
                                        fieldId: 'custbody8'
                                    });
                                    var oldThirdApprover = oldBillRec.getValue({
                                        fieldId: 'custbody9'
                                    });
                                    var oldFourthApprover = oldBillRec.getValue({
                                        fieldId: 'custbody10'
                                    });
                                    var oldFinanceApprover = oldBillRec.getValue({
                                        fieldId: 'custbody_crw_finance_approver'
                                    });
                                    var oldItemLines = oldBillRec.getLineCount({
                                        sublistId: 'item'
                                    });
                                    var oldExpenselines = oldBillRec.getLineCount({
                                        sublistId: 'expense'
                                    });
                                    ...
                                    if (itemLines > 0 && isEdited != true) {
                                        for (var x = 0; x < itemLines; x++) {
                                            var item = billRecObj.getSublistValue({
                                                sublistId: 'item',
                                                fieldId: 'item',
                                                line: x
                                            });
                                            var lineCC = billRecObj.getSublistValue({
                                                sublistId: 'item',
                                                fieldId: 'department',
                                                line: x
                                            });
                                            var propHier = billRecObj.getSublistValue({
                                                sublistId: 'item',
                                                fieldId: 'class',
                                                line: x
                                            });
                                            var oldItem = oldBillRec.getSublistValue({
                                                sublistId: 'item',
                                                fieldId: 'item',
                                                line: x
                                            });
                                            var oldLineCC = oldBillRec.getSublistValue({
                                                sublistId: 'item',
                                                fieldId: 'department',
                                                line: x
                                            });
                                            var oldPropHier = oldBillRec.getSublistValue({
                                                sublistId: 'item',
                                                fieldId: 'class',
                                                line: x
                                            });
                                    ...
                                    if (expenselines > 0 && isEdited != true) {
                                        for (var x = 0; x < expenselines; x++) {
                                            var account = billRecObj.getSublistValue({
                                                sublistId: 'expense',
                                                fieldId: 'account',
                                                line: x
                                            });
                                            ...
                                            var oldAccount = oldBillRec.getSublistValue({
                                                sublistId: 'expense',
                                                fieldId: 'account',
                                                line: x
                                            });
                                ...
                                if (((scriptContext.type == scriptContext.UserEventType.CREATE && billStatus == 1) && (!isEmpty(firstApprover))) || (isEdited == true && (!isEmpty(firstApprover)))) {
                                    ...
                                    var secondApproverJson = runtime.getCurrentScript().getParameter({
                                        name: 'custscript_crw_bill_second_approver'
                                    });
                                    var thirdApproverJson = runtime.getCurrentScript().getParameter({
                                        name: 'custscript_crw_bill_third_approver'
                                    });
                                    var fourthApproverJson = runtime.getCurrentScript().getParameter({
                                        name: 'custscript_crw_bill_fourth_approver'
                                    });
                                    var financeApproverJson = runtime.getCurrentScript().getParameter({
                                        name: 'custscript_crw_bill_finance_approver'
                                    });
                                    ...
                                    if (!isEmpty(secondApproverJson) && !isEmpty(thirdApproverJson) && !isEmpty(fourthApproverJson) && !isEmpty(financeApproverJson)) {
                                        ...
                                        if (totalAmount > 50000) {
                                            ...
                                            var secondAppr = billRecObj.getValue({
                                                fieldId: 'custbody8'
                                            });
                                            var thirdAppr = billRecObj.getValue({
                                                fieldId: 'custbody9'
                                            });
                                        ...
                                        if (totalAmount > 500000) {
                                            ...
                                            var fourthAppr = billRecObj.getValue({
                                                fieldId: 'custbody10'
                                            });
                                        ...
                                        billRecObj.setValue({
                                            fieldId: 'custbody_crw_next_approver',
                                            value: firstApprover
                                        });
                                        billRecObj.setValue({
                                            fieldId: 'custbody_last_approved_by',
                                            value: ''
                                        });
                                        billRecObj.setValue({
                                        ...
                                        billRecObj.setValue({
                                            fieldId: 'custbody_crw_approved_by_via_netsuite',
                                            value: ''
                                        });
                                        billRecObj.setValue({
                                            fieldId: 'custbody_rejected_by',
                                        ...
                                        billRecObj.setValue({
                                            fieldId: 'custbody_crw_po_appr_passcode',
                                            value: JSON.stringify(passcode)
                                        });
                                        billRecObj.setValue({
                                            fieldId: 'custbody_crw_approvers_skipped',
                                            value: ''
                                        });

                                        if (isEdited == true) {
                                            billRecObj.setValue({
                                                fieldId: 'custbody_crw_po_approval_edited',
                                                value: true
                                            });
                                            billRecObj.setValue({
                                                fieldId: 'custbody11',
                                            ...
                                            emailauthor = billRecObj.getValue({
                                                fieldId: 'custbody_createdby'
                                            });
                                            billRecObj.setValue({
                                                fieldId: 'approvalstatus',
                                        ...
                                        var recordSaved = billRecObj.save({
                                        ...
                                        if (itemLines > 0) {
                                            ...
                                            for (var p = 0; p < itemLines; p++) {
                                                tableData += '<tr><td>' + billRecObj.getSublistText({
                                                    sublistId: 'item',
                                                    fieldId: 'item',
                                                    line: p
                                                }); + '</td>';
                                                tableData += '<td>' + billRecObj.getSublistValue({
                                                    sublistId: 'item',
                                                    fieldId: 'description',
                                                    line: p
                                                }); + '</td>';
                                                ...
                                                tableData += '<td>' + billRecObj.getSublistText({
                                                    sublistId: 'item',
                                                    fieldId: 'amount',
                                                    line: p
                                                }); + '</td>';
                                                tableData += '<td>' + billRecObj.getText({
                                                    fieldId: 'currency',
                                                    line: p
                                                }); + '</td>';
                                                tableData += '<td>' + billRecObj.getSublistText({
                                        ...
                                        if (!isEmpty(billFileObjArray)) {
                                            ...
                                            var billFile = render.transaction({
                                                entityId: recId,
            ...
            try {
                ...
                if (searchObject != null && searchObject != '' && searchObject != undefined) {
                    ...
                    if (searchResults.length >= 1) {
                        internalId = searchResults[0].getValue({
                            name: 'internalid',
                            join: 'file'
                        });
                    }
